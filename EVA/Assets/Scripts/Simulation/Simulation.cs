...
public class Simulation : MonoBehaviour {
	...
	void SaveInitialParameters(string file) {
		Logger logger = Logger.getInstance();
		logger.File = file;

		logger.Save("Initial parameters of the simulation");
		logger.Save(JsonConvert.SerializeObject(new SerializableParameters()));
	}

	// Use this for initialization
	void Start () {
		System.DateTime now = System.DateTime.Now;
		SaveInitialParameters("Initial_Parameters-" + now.Day + "_" + now.Month + "_" + now.Year + "-" + now.Hour + "_" + now.Minute + "_" + now.Second + ".txt");
